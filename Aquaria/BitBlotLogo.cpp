...
void BitBlotLogo::applyState()
{
	...
	if (rand()%100 < 40)
	{
		...
		for (unsigned i = 0; i < dragon->bones.size(); i++)
		{
	...
	for (int i = 0; i < numWindmills; i++)
	{
		...
		for (unsigned i = 0; i < windmill->bones.size(); i++)
		{
	...
	if (windmills.size() >= 2)
	{
		for (unsigned i = 0; i < windmills.size()-1; i++)
		{
			if (windmills[i]->position.y < 4000)
			{
				for (unsigned j = i+1; j < windmills.size(); j++)
				{
